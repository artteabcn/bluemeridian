...
export const GET: APIRoute = async ({ params, locals }) => {
  ...
  const { results } = await db.prepare('SELECT * FROM company_documents WHERE id = ?').bind(Number(docId)).all();
...
export const POST: APIRoute = async ({ params, request, locals, redirect }) => {
  ...
  if (String(formData.get('action')) === 'delete') {
    const { results } = await db.prepare('SELECT r2_key FROM company_documents WHERE id = ?').bind(Number(docId)).all();
    const doc = results[0] as { r2_key: string } | undefined;
    if (doc) {
      await r2.delete(doc.r2_key);
      await db.prepare('DELETE FROM company_documents WHERE id = ?').bind(Number(docId)).run();
    }
  }
